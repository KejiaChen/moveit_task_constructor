    ...
    def read(self, db: str, collection: str, search_param: dict) -> list:
        ...
        for key in search_param:
            ...
            if isinstance(value, str) and value.endswith('*'):
                # if with * at the end, search for all the contend starting with the given string
                pattern = value.rstrip('*')
                search_param[key] = {"$regex": f"^{pattern}", "$options": "i"}  # Case-insensitive starts-with match
